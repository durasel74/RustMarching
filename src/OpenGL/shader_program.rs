...
impl ShaderProgram {
    ...
    pub fn from_shaders(vert_shader: &Shader, frag_shader: &Shader) -> Result<Self, ShaderError> {
        ...
        Ok(ShaderProgram { 
            id: link_result, vert_id: vert_shader.id(), 
    ...
    fn get_link_result(id: u32) -> Result<u32, ShaderError>{
        ...
        if success == 0 {
            ...
            let error = super::misc::create_string_buffer(len as usize);
            unsafe {
    ...
    pub fn set_uniform_vector2(&self, field_name: &str, value: &Vector2<f32>) {
        let cfield_name = ffi::CString::new(field_name).unwrap();
        unsafe {
            let location = gl::GetUniformLocation(self.id(), cfield_name.as_ptr());
