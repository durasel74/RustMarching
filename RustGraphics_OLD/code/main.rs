...
use objects::{ RenderObject, Mesh, Camera, ViewPort, Light, LightType, 
    FrameBuffer, CubeMap, ShaderProgram, obj_loader, Vertex };
...
fn main() {
    ...
    let sky_box_vert_filename = Path::new("Shaders/skybox.vert").to_str().unwrap();
    let sky_box_frag_filename = Path::new("Shaders/skybox.frag").to_str().unwrap();
    ...
    let shader_loadresult = objects::ShaderProgram::from_files(
        sky_box_vert_filename, sky_box_frag_filename);
    let sky_box_shader_program = match shader_loadresult {
        Ok(program) => program,
        Err(err) => { println!("{}", err); return }
    };
    ...
    let sky_box = CubeMap::from_folder("./CubeMap/skybox").unwrap();
    let sky_box_mesh = create_sky_box_mesh("Models/SkyBox/Model.obj");
        ...
        *control_flow = event_loop::ControlFlow::Poll;
        match event {
            ...
            event::Event::MainEventsCleared => {
                ...
                // view_port.draw_sky_box(&sky_box_shader_program, &mut camera, &sky_box, &sky_box_mesh);
                view_port.draw_sky_box(&shader_program, &mut camera, &sky_box, &sky_box_mesh);
...
fn create_sky_box_mesh(model_path: &str) -> Mesh {
    let model_path = Path::new(model_path).to_str().unwrap();
    let rend_obj = obj_loader::load_model(model_path);
    rend_obj.meshes()[0].clone()
}
